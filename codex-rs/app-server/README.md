...
- `thread/search` — search persisted SQLite task names, titles, and previews with a literal, case-sensitive substring; returns the metadata preview as `snippet` with the existing cursor, archive, and source filters. Requires the state database; unavailable metadata is an error, without a rollout or legacy name-index scan. Use `thread/searchOccurrences` for body matches within one paginated thread.
