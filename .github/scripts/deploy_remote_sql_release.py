...
def github_json(url: str, token: str) -> dict[str, Any]:
    payload = github_json_any(url, token)
    if not isinstance(payload, dict):
...
def github_release_by_tag(repository: str, token: str, release_tag: str) -> dict[str, Any]:
    api_root = "https://api.github.com"
    page = 1
    while True:
        payload = github_json_any(
            f"{api_root}/repos/{repository}/releases?per_page=100&page={page}",
            token,
        )
        if not isinstance(payload, list):
            raise DeployError("GitHub API release listing was not an array")
        if not payload:
            break
        for release in payload:
            if isinstance(release, dict) and release.get("tag_name") == release_tag:
                return release
        page += 1
    raise DeployError("draft GitHub Release could not be found by tag")
...
def release_assets(args: argparse.Namespace, directory: Path) -> dict[str, Path]:
    ...
    api_root = "https://api.github.com"
    release = github_release_by_tag(args.repository, token, args.release_tag)
...
def select_postgres_pod(args: argparse.Namespace, *, timeout: int = 300) -> str:
    deadline = time.monotonic() + timeout
    last_candidates: list[str] = []
    while True:
        pods = kubectl_json(
            args, "get", "pods", "-l", "app.kubernetes.io/name=codex-postgres"
        ).get("items", [])
        if not isinstance(pods, list):
            raise DeployError("postgres pod list response omitted items")
        candidates: list[str] = []
        for pod in pods:
            if not isinstance(pod, dict) or not pod_is_ready(pod, "postgres"):
                continue
            name = pod.get("metadata", {}).get("name")
            if isinstance(name, str):
                candidates.append(name)
        ...
        if time.monotonic() >= deadline:
            raise DeployError(
                "expected exactly one ready postgres pod; found " f"{last_candidates}"
            )
        time.sleep(5)


def workspace_cat_command(namespace: str, pod: str, container: str, destination: str) -> str:
    return (
        "kubectl -n "
        f"{shlex.quote(namespace)} exec -i {shlex.quote(pod)} -c {shlex.quote(container)} "
        "-- sh -lc 'cat > \"$1\"' -- "
        f"{shlex.quote(destination)}"
    )


def postgres_dump_command(namespace: str, postgres_pod: str) -> str:
    return (
        "kubectl -n "
        f"{shlex.quote(namespace)} exec {shlex.quote(postgres_pod)} -c postgres "
        "-- sh -lc "
        "'CODEX_REMOTE_SQL_URL=\"$1\"; export CODEX_REMOTE_SQL_URL; "
        "exec pg_dump --dbname=\"$CODEX_REMOTE_SQL_URL\" "
        "--format=custom --no-owner --no-privileges' -- "
        '"$CODEX_REMOTE_SQL_URL"'
    )


def postgres_psql_command(namespace: str, postgres_pod: str, sql: str) -> str:
    return (
        "kubectl -n "
        f"{shlex.quote(namespace)} exec {shlex.quote(postgres_pod)} -c postgres -- sh -lc "
        "'CODEX_REMOTE_SQL_URL=\"$1\"; SQL_QUERY=\"$2\"; export CODEX_REMOTE_SQL_URL; "
        "exec psql -XAtq -v ON_ERROR_STOP=1 --dbname=\"$CODEX_REMOTE_SQL_URL\" -c \"$SQL_QUERY\"' -- "
        '"$CODEX_REMOTE_SQL_URL" '
        f"{shlex.quote(sql)}"
    )


def postgres_restore_list_command(namespace: str, postgres_pod: str) -> str:
    return (
        "kubectl -n "
        f"{shlex.quote(namespace)} exec -i {shlex.quote(postgres_pod)} -c postgres "
        "-- pg_restore --list -"
    )
...
[ "${version_output}" = "${selected_version}" ]
...
'''
    ...
    if not values["VERSION_OUTPUT"].startswith("codex-cli "):
...
'''
    ...
    values = parse_safe_output(setup_result.stdout, {"BACKUP_STAGE", "BACKUP_DIR"})
    stage = values["BACKUP_STAGE"]
    destination = values["BACKUP_DIR"]
    db_url_result = remote_exec(
        args,
        pod,
        r'''set -eu
printf '%s\n' "${CODEX_REMOTE_SQL_URL:-}"
''',
    ...
    database_url = db_url_result.stdout.strip()
    if not database_url:
        raise DeployError("remote SQL URL was not available in the workspace pod")
    host_env = os.environ.copy()
    host_env["CODEX_REMOTE_SQL_URL"] = database_url
    postgres_pod = select_postgres_pod(args)
    dump_destination = f"{stage}/postgres.dump"
    live_sqlx_destination = f"{stage}/live-sqlx-migrations.txt"
    pg_restore_list_destination = f"{stage}/pg_restore.list"
    psql_sql = (
        "SELECT version::text || ':' || encode(checksum, 'hex') "
        "FROM _sqlx_migrations "
        "WHERE version BETWEEN 1 AND 7 AND success "
        "ORDER BY version"
    )
    dump_pipeline = (
        "set -euo pipefail\n"
        f"{postgres_dump_command(args.namespace, postgres_pod)} | "
        f"{workspace_cat_command(args.namespace, pod, args.container, dump_destination)}\n"
    )
    run(["bash", "-lc", dump_pipeline], env=host_env, timeout=args.pg_backup_timeout)
    psql_pipeline = (
        "set -euo pipefail\n"
        f"{postgres_psql_command(args.namespace, postgres_pod, psql_sql)} | "
        f"{workspace_cat_command(args.namespace, pod, args.container, live_sqlx_destination)}\n"
    )
    run(["bash", "-lc", psql_pipeline], env=host_env, timeout=args.pg_backup_timeout)
    restore_list_pipeline = (
        "set -euo pipefail\n"
        f"{workspace_cat_command(args.namespace, pod, args.container, dump_destination)} | "
        f"{postgres_restore_list_command(args.namespace, postgres_pod)} | "
        f"{workspace_cat_command(args.namespace, pod, args.container, pg_restore_list_destination)}\n"
    )
    run(["bash", "-lc", restore_list_pipeline], env=host_env, timeout=args.pg_backup_timeout)
...
[ -s "${stage}/live-sqlx-migrations.txt" ]
