...
def require_started_model_provider(
    ...
    started_model = response.get("model")
    if not isinstance(started_model, str) or not started_model:
        raise SmokeError("thread/start did not report a model")
    started_provider = response.get("modelProvider")
    if not isinstance(started_provider, str) or not started_provider:
        raise SmokeError("thread/start did not report a model provider")
    if model_name is not None and started_model != model_name:
        raise SmokeError("thread/start used an unexpected model")
    if model_provider is not None and started_provider != model_provider:
        raise SmokeError("thread/start used an unexpected model provider")
    return started_model, started_provider


def require_legacy_history_mode(thread: dict[str, Any], label: str) -> None:
    history_mode = thread.get("historyMode")
    if history_mode not in (None, "legacy"):
        raise SmokeError(f"{label} used an unexpected history mode")
...
def wait_for_turn(
    ...
    while True:
        ...
        if remaining <= 0:
            raise SmokeError("smoke turn did not complete before timeout")
        ...
        if turn.get("status") != "completed":
            raise SmokeError(f"smoke turn ended with status {turn.get('status')!r}")
        ...
        if messages != [sentinel] and not turn_contains_exact_message(turn, sentinel):
            raise SmokeError("smoke turn did not return the exact sentinel")
        return


def thread_start_params(args: argparse.Namespace) -> dict[str, Any]:
    params: dict[str, Any] = {
        "cwd": args.cwd,
...
def pre_restart(args: argparse.Namespace) -> None:
    ...
    sentinel = f"REMOTE_SQL_SMOKE_OK:{nonce}"
    ...
    with ProxyWebSocketClient(proxy_command(args), args.request_timeout_seconds) as client:
        ...
        require_legacy_history_mode(known_thread, "known thread resume")
        ...
        started = client.request("thread/start", thread_start_params(args))
        started_model, started_provider = require_started_model_provider(
        ...
        if not any(turn.get("id") == turn_id for turn in persisted_turns):
            raise SmokeError("smoke turn was not persisted")
    ...
    state = {
        ...
        "sentinel": sentinel,
        "goal": expected_goal,
        "startedModel": started_model,
        "startedProvider": started_provider,
        "createdThreadIds": created_threads,
    }
        ...
        json.dumps(
            {
                ...
                "newTask": True,
                "historyModeLegacyFallback": True,
                "modelReported": True,
...
def post_restart(args: argparse.Namespace) -> None:
    ...
    with ProxyWebSocketClient(proxy_command(args), args.request_timeout_seconds) as client:
        client.initialize()
        resumed_smoke_thread = require_thread(
            client.request(
                "thread/resume", {"threadId": smoke_thread_id, "excludeTurns": True}
            ),
            smoke_thread_id,
        )
        require_started_model_provider(
            {
                "model": resumed_smoke_thread.get("model"),
                "modelProvider": resumed_smoke_thread.get("modelProvider"),
            },
            str(state["startedModel"]),
            str(state["startedProvider"]),
        )
        ...
        if not isinstance(matching_turn, dict) or not turn_contains_exact_message(
            ...
            raise SmokeError("smoke turn content did not survive daemon restart")
        goal = client.request(
            "thread/goal/get", {"threadId": smoke_thread_id}
        ).get("goal")
        if not isinstance(goal, dict):
        ...
        require_legacy_history_mode(known_thread, "known thread restart resume")
